...
def create_graph(response_jsons, name='unnamed', retain_all=True, bidirectional=False):
    ...
    if len(elements) < 1:
        raise ox.EmptyOverpassResponse('There are no data elements in the response JSON objects')
...
punggol = gpd2.read_file('polygon-punggol.geojson')
polygon = punggol['geometry'].iloc[0]


busstop_Query = '[out:json];(node["highway"="bus_stop"](1.3891,103.8872,1.4222,103.9261);>;);out;'
responsejson_Busstop = ox.overpass_request(data={'data': busstop_Query}, timeout=180)
busstop_Graph = create_graph(responsejson_Busstop)
busstop_Graph = ox.truncate_graph_polygon(busstop_Graph, polygon, truncate_by_edge=True,  retain_all=True)

node, edge = ox.graph_to_gdfs(busstop_Graph)
node.replace(' ', np.nan, inplace=True)
node.to_csv("bus_stop.csv")
# node.to_csv(node)